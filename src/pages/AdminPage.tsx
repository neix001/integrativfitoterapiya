...
import { Plus, Edit, Trash2, Users, MessageSquare, BarChart3, Settings } from 'lucide-react';
...
const AdminPage: React.FC = () => {
  ...
  const tabs = [
    ...
    { id: 'blog', label: language === 'en' ? 'Blog Posts' : language === 'az' ? 'Bloq Yazıları' : 'Посты Блога', icon: Edit },
    { id: 'programs', label: language === 'en' ? 'Diet Programs' : language === 'az' ? 'Diet Proqramları' : 'Диетические Программы', icon: Plus },
    { id: 'classes', label: language === 'en' ? 'Live Classes' : language === 'az' ? 'Canlı Dərslər' : 'Живые Занятия', icon: MessageSquare },
    { id: 'users', label: language === 'en' ? 'Users' : language === 'az' ? 'İstifadəçilər' : 'Пользователи', icon: Users },
    { id: 'support', label: language === 'en' ? 'Support' : language === 'az' ? 'Dəstək' : 'Поддержка', icon: Settings }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-6">
                {language === 'en' ? 'Recent Activity' : language === 'az' ? 'Son Fəaliyyət' : 'Недавняя Активность'}
              </h2>
              <div className="space-y-4">
                {users.slice(0, 5).map((user) => (
                  <div key={user.id} className="flex items-center space-x-4 p-4 border border-gray-200 rounded-xl">
        ...
        {activeTab === 'blog' && (
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="space-y-4">
              {blogPosts.map((post) => (
                <div key={post.id} className="border border-gray-200 rounded-xl p-4 hover:border-green-300 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 mb-2">{post.title[language]}</h3>
                      <p className="text-gray-600 text-sm mb-2">{post.excerpt[language]}</p>
        ...
        {activeTab === 'programs' && (
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">
                {t('admin.manage_programs', language)}
              </h2>
              <button className="bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 transition-colors flex items-center space-x-2">
                <Plus className="w-4 h-4" />
                <span>{t('admin.add_new', language)}</span>
        ...
        {activeTab === 'classes' && (
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">
                {t('admin.manage_classes', language)}
              </h2>
              <button className="bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 transition-colors flex items-center space-x-2">
                <Plus className="w-4 h-4" />
                <span>{t('admin.add_new', language)}</span>
            ...
            <div className="space-y-4">
              {liveClasses.map((liveClass) => (
                <div key={liveClass.id} className="border border-gray-200 rounded-xl p-4 hover:border-green-300 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 mb-2">{liveClass.title[language]}</h3>
                      <p className="text-gray-600 text-sm mb-2">{liveClass.description[language]}</p>
                      <div className="flex items-center space-x-4 text-sm text-gray-500">
                        <span>{liveClass.date}</span>
                        <span>{liveClass.time}</span>
                        <span>{liveClass.instructor}</span>
                        <span>${liveClass.price}</span>
                        <span>{liveClass.currentParticipants}/{liveClass.maxParticipants}</span>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2 ml-4">
                      <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
